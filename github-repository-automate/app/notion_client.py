...
class NotionClient:
    ...
    async def clear_repo_id(self, page_id: str) -> None:
        ...
        await self._patch_page(page_id, {
            "properties": {settings.notion_prop_repo_id: {"number": None}}
        })
    ...
    async def query_pages_by_name(self, name: str) -> list[dict]:
        ...
        body = {
            "filter": {
                "property": settings.notion_prop_name,
                "title": {"equals": name},
            },
        }
    ...
    def get_page_name(page: dict) -> str:
        ...
        name_obj = props.get(settings.notion_prop_name, {})
        title_arr = name_obj.get("title", [])
        return title_arr[0]["plain_text"] if title_arr else ""

    @staticmethod
    def get_page_repo_id(page: dict) -> int | None:
